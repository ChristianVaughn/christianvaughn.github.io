...
const Header: React.FC<INavBar> = ({ navs, className }) => {
  ...
  return (
    <header
      className={`v-header-2 ${className ? (className ? className : className) : ""}${isScrolled ? (isAtBottom ? "bottom-header" : "sticky-header") : ""}`}
    >
      {/* navbar toggler */}
      <Navbar.Toggle
          ...
          {navs?.map((item: INavs, idx: number) => {
            ...
            return (
              <li key={idx}>
                ...
                >
                  <i
                    className={`${item?.icon} avatar rounded-circle bg-gray-100 link-circle fs-18`}
                  ></i>
                  <span> {item.text}</span>
                </ScrollLink>
              </li>
